...
function AddandHandle(a: number, b: number, cb: (num: number) => void) { // Function with callback cb as function
    const result = a + b;
    cb(result);
}
...
AddandHandle(10, 20, (result) => {
    console.log(result);
    return result; // TypeScript will not do anything with this return value
}
);